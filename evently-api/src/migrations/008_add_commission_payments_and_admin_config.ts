...
export async function up(conn: typeof mongoose) {
    ...
    await db
        ...
        .createIndex({ vendorId: 1, monthKey: 1, status: 1 });
    await db
        .collection("commissionPayments")
...
export async function down(conn: typeof mongoose) {
    const db = conn.connection.db;
    if (!db) throw new Error("Database connection is not ready");

    await db.collection("commissionPayments").drop().catch(() => undefined);
    await db.collection("adminPaymentConfigs").drop().catch(() => undefined);
}
