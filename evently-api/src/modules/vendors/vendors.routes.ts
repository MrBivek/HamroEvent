...
vendorsRoutes.post(
    ...
    async (req, res, next) => {
        try {
            ...
            const profile = await hydrateVendorProfile(vendor.toObject(), {
        ...
        if (query.locationId && mongoose.isValidObjectId(query.locationId)) {
            ...
            andFilters.push({
                $or: [
                    ...
                    { serviceAreas: { $in: [query.location] } },
                ],
            });
        }
                ...
                for (const entry of withSlots) {
                    const fits =
                        ...
                        entry.slots?.some((slot) =>
                            rangeWithinSlot(timeRange, slot.start, slot.end),
                        );
                    if (!fits) {
